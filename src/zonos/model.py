...
class Zonos(nn.Module):
    ...
    def _decode_one_token(
        ...
        if not allow_cudagraphs or input_ids.device.type != "cuda":
            ...
            return self._compute_logits(
                hidden_states_local, inference_params, cfg_scale
            )
        ...
        if need_capture:
            ...
            for _ in range(3):
                ...
                logits = self._compute_logits(
                    hidden_states, inference_params, cfg_scale
                )
    ...
    def generate(
        ...
        audio_prefix_codes: (
            torch.Tensor | None
        ) = None,  # [bsz, 9, prefix_audio_seq_len]
        ...
        logit_bias[:, 1:, self.eos_token_id] = (
            -torch.inf
        )  # only allow codebook 0 to predict EOS
